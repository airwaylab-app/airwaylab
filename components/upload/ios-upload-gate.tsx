'use client';

import { useEffect, useRef, useState } from 'react';
import Link from 'next/link';
import { AlertCircle, CheckCircle2, Loader2, Monitor } from 'lucide-react';
import { Card, CardHeader, CardTitle, CardContent } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { events } from '@/lib/analytics';

export function IosUploadGate() {
  const [email, setEmail] = useState('');
  const [consent, setConsent] = useState(false);
  const [status, setStatus] = useState<'idle' | 'sending' | 'success' | 'error'>('idle');
  const [suppressed, setSuppressed] = useState(true); // default true to avoid SSR flash
  const emailInputRef = useRef<HTMLInputElement>(null);

  useEffect(() => {
    if (localStorage.getItem('airwaylab_remind_submitted')) {
      setSuppressed(true);
    } else {
      setSuppressed(false);
      events.mobileReminderShown();
    }
  }, []);

  async function handleSubmit() {
    if (!email.includes('@') || !consent || status !== 'idle') return;
    setStatus('sending');
    events.mobileReminderSubmitted();
    try {
      const res = await fetch('/api/remind-desktop', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, consent: true }),
      });
      if (res.ok) {
        localStorage.setItem('airwaylab_remind_submitted', Date.now().toString());
        setStatus('success');
        events.mobileReminderSuccess();
      } else {
        setStatus('error');
        emailInputRef.current?.focus();
        events.mobileReminderError();
      }
    } catch {
      setStatus('error');
      emailInputRef.current?.focus();
      events.mobileReminderError();
    }
  }

  if (status === 'success') {
    return (
      <Card>
        <CardContent className="pt-4 text-center">
          <CheckCircle2 className="h-8 w-8 text-emerald-500 mx-auto mb-3" aria-hidden="true" />
          <p className="text-base font-semibold text-foreground">Check your inbox</p>
          <p className="text-sm text-muted-foreground mt-1">
            We&apos;ll send you the link within 24 hours. It works on any desktop browser.
          </p>
          <Link
            href="/analyze?demo"
            className="mt-4 inline-block text-sm text-primary hover:underline"
          >
            Try the demo instead
          </Link>
        </CardContent>
      </Card>
    );
  }

  return (
    <Card>
      <CardHeader>
        <div className="flex items-center gap-2">
          <Monitor className="h-5 w-5 text-primary" aria-hidden="true" />
          <CardTitle>Upload needs a desktop browser</CardTitle>
        </div>
        <p className="text-sm text-muted-foreground mt-1">
          SD card folder selection requires a desktop. We&apos;ll send you a quick reminder link.
        </p>
      </CardHeader>
      <CardContent>
        {!suppressed && (
          <>
            <label htmlFor="ios-gate-email" className="sr-only">
              Email address
            </label>
            <input
              ref={emailInputRef}
              id="ios-gate-email"
              type="email"
              placeholder="your@email.com"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              disabled={status === 'sending'}
              className="h-10 w-full rounded-md border border-border bg-background px-3 text-sm placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-primary/50 disabled:opacity-50"
            />
            <div className="flex items-start gap-2 mt-3 text-left">
              <input
                type="checkbox"
                id="ios-gate-consent"
                checked={consent}
                onChange={(e) => setConsent(e.target.checked)}
                disabled={status === 'sending'}
                className="mt-0.5 h-4 w-4 shrink-0 accent-primary"
              />
              <label htmlFor="ios-gate-consent" className="text-xs text-muted-foreground cursor-pointer">
                I agree to receive a one-time reminder email. No marketing. Unsubscribe any time.
              </label>
            </div>
            <Button
              onClick={handleSubmit}
              disabled={!email.includes('@') || !consent || status !== 'idle'}
              aria-label={status === 'sending' ? 'Sending reminder...' : undefined}
              className="mt-4 w-full"
            >
              {status === 'sending' ? (
                <>
                  <Loader2 className="h-4 w-4 animate-spin mr-2" aria-hidden="true" /> Sending...
                </>
              ) : (
                'Remind me'
              )}
            </Button>
            {status === 'error' && (
              <p className="text-xs text-red-400 flex items-center gap-1.5 mt-2 justify-center">
                <AlertCircle className="h-3.5 w-3.5" aria-hidden="true" />
                Something went wrong — try again.
              </p>
            )}
          </>
        )}
        <div className="mt-4 text-center">
          <Link
            href="/analyze?demo"
            className="text-sm text-primary hover:underline"
          >
            Try the demo instead
          </Link>
        </div>
      </CardContent>
    </Card>
  );
}
