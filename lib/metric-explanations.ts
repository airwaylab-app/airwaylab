...
export function getNEDExplanation(nedMean: number, reraIndex: number, nedThreshold: ThresholdDef): string {
  ...
  if (reraIndex >= 10) {
    return `Your RERA index of ${fmt(reraIndex)}/hr means your breathing effort increases frequently before triggering a brief arousal. These events are not captured by standard AHI counting.`;
  }
  if (light === 'bad') {
    return `Your NED mean of ${fmt(nedMean)}% shows mid-inspiratory flow reduction throughout the night.`;
  }
