...
function singleNightInsights(n: NightResult, prev: NightResult | null, symptomRating?: number | null): Insight[] {
  ...
  if (iflL === 'bad') {
    ...
    insights.push({
      ...
      body: `Your IFL Symptom Risk of ${fmt(iflRisk)}% is low, with flow limitation metrics in the typical range.`,
      category: 'ned',
    });
  }
  ...
  if (gl === 'good') {
    insights.push({
      ...
      body: `Score of ${fmt(n.glasgow.overall)} indicates minimal breath-shape abnormality, within the typical range.`,
      category: 'glasgow',
    });
  } else if (gl === 'bad') {
    insights.push({
  ...
  if (regL === 'bad') {
    insights.push({
      ...
      body: `Regularity score of ${Math.round(n.wat.regularityScore)}% indicates very predictable breathing cycles, which is outside the typical variability range.`,
      category: 'wat',
    });
  } else if (regL === 'good') {
    insights.push({
  ...
  if (eaiL === 'bad') {
    insights.push({
      ...
      body: `RDI of ${fmt(eaiVal)}/hr indicates a high rate of recovery breaths following flow-limited breathing. Your clinician can help interpret these findings in context. Get deeper observations with AI Analysis.`,
      category: 'ned',
    });
  ...
  if (glasgowVal <= 2.0 && eaiVal >= 15) {
    insights.push({
      ...
      body: `Glasgow Index of ${fmt(glasgowVal)} shows mild flow limitation, but RDI of ${fmt(eaiVal)}/hr indicates frequent disruptions. This mismatch between flow limitation and disruption frequency is a pattern worth noting. Log your night context to track patterns and discuss these findings with your clinician.`,
      category: 'ned',
  ...
  if (gl === 'good' && nedL === 'good' && (watFLL === 'bad' || watFLL === 'warn')) {
    insights.push({
      ...
      body: `WAT FL Score of ${Math.round(n.wat.flScore)}% is elevated while Glasgow (${fmt(n.glasgow.overall)}) and NED (${fmt(n.ned.nedMean)}%) are in range. These engines measure different aspects of breath shape -- WAT focuses on waveform flatness, while Glasgow and NED use other criteria. This divergence is a common pattern worth noting.`,
      category: 'wat',
    });
  ...
  if (
    ...
    insights.push({
      ...
      body: 'RERA index is low while flow limitation metrics are elevated. These metrics measure different aspects of breathing. Discuss these findings with your clinician.',
      category: 'therapy',
    });
  }
  ...
  if (n.ned.reraIndex >= 10) {
    ...
    insights.push({
      ...
      body: `NED mean of ${fmt(n.ned.nedMean)}% with only ${fmt(n.ned.reraIndex)} RERAs/hr, both within the typical range.`,
      category: 'ned',
    });
  }
  ...
  if (Math.abs(n.ned.h1NedMean - n.ned.h2NedMean) > 5) {
    ...
    insights.push({
      ...
      body: `H1 NED ${fmt(n.ned.h1NedMean)}% vs H2 NED ${fmt(n.ned.h2NedMean)}%. ${worse === 'second' ? 'This half-night pattern is common and worth noting over multiple nights.' : 'Early-night values are higher than the second half.'}`,
      category: 'ned',
    });
  }
  ...
  if (boiVal > 5) {
    ...
    insights.push({
      ...
      body: `Brief obstruction rate of ${fmt(boiVal)}/hr means brief airway narrowing roughly every ${interval} minutes. These events are shorter than standard detection thresholds. Track your sleep position and note patterns. Discuss these findings with your clinician.`,
      category: 'ned',
    });
  }
  ...
  if (boiH2 > boiH1 * 1.5 && boiH2 > 3) {
    insights.push({
      ...
      body: `Brief obstructions are higher in H2 (${fmt(boiH2)}/hr) vs H1 (${fmt(boiH1)}/hr). This half-night pattern is worth tracking across multiple nights.`,
      category: 'ned',
    });
  }
    ...
    if (prev?.settingsMetrics) {
      ...
      if (Math.abs(tiDelta) > 150) {
        insights.push({
          ...
          body: `Ti shifted by ${tiDelta > 0 ? '+' : ''}${Math.round(tiDelta)}ms compared to last night (${prev.settingsMetrics.tiMedianMs}ms → ${sm.tiMedianMs}ms). This change is outside typical night-to-night variation.`,
          category: 'settings',
        });
      }
    }
  ...
  if (n.oximetry) {
    ...
    if (ox.tBelow94 > 15 && ox.odi3 < 5) {
      insights.push({
        ...
        body: `${fmt(ox.tBelow94)}% of time below 94% SpO2 but only ${fmt(ox.odi3)} desaturation events/hr. This pattern shows sustained lower oxygen levels rather than repeated discrete desaturation events. If this persists, discuss with your clinician.`,
        category: 'oximetry',
      });
    }
    ...
    if (ox.coupled3_10 > 10) {
      insights.push({
        ...
        body: `${fmt(ox.coupled3_10)} coupled events/hr -- desaturation and heart rate surge occurring within 30 seconds of each other. This temporal coupling pattern differs from independent HR surges and desaturations.`,
        category: 'oximetry',
      });
    }
    ...
    if (Math.abs(ox.h1.odi3 - ox.h2.odi3) > 5) {
      ...
      insights.push({
        ...
        body: `H1 ODI-3: ${fmt(ox.h1.odi3)}/hr vs H2: ${fmt(ox.h2.odi3)}/hr. This half-night pattern is worth tracking across multiple nights.`,
        category: 'oximetry',
      });
    }
  }
  ...
  if (n.crossDevice) {
    ...
    if (cd.couplingPct > 50) {
      ...
      insights.push({
        ...
        body: `Only ${fmt(cd.couplingPct)}% of your breathing events caused arousals, indicating low coupling between breathing events and heart rate responses.`,
        category: 'correlation',
      });
    }
  ...
  if (ms) {
    if (ms.ahi != null && ms.ahi > 5) {
      insights.push({
        ...
        body: `Your device reported ${fmt(ms.ahi)} apnea/hypopnea events per hour. ${ms.ahi > 10 ? 'This is significantly above the typical range.' : 'This is moderately elevated.'} Discuss with your clinician.`,
        category: 'therapy',
      });
    }
    ...
    if (ms.leak95 != null && ms.leak95 > 40) {
      insights.push({
        ...
        body: `95th percentile leak of ${fmt(ms.leak95, 0)} L/min. High leak values can make flow data less reliable. This is above the typical range.`,
        category: 'therapy',
      });
    }
