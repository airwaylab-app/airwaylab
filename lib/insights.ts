...
function singleNightInsights(n: NightResult, prev: NightResult | null, symptomRating?: number | null): Insight[] {
  ...
  if (boiVal > 5) {
    ...
    insights.push({
      ...
      body: `Brief obstruction rate of ${fmt(boiVal)}/hr — brief obstruction events appear roughly every ${interval} minutes in your data. These events are too short for standard detection but may fragment sleep. Tracking your sleep position may help identify patterns. Your clinician can help interpret these findings in context.`,
      category: 'ned',
    });
  }
