-- ============================================================
-- 058: subscription_events dedup guard (ST1, BLOCKER 2 defence-in-depth)
...
-- row per Stripe event via a PARTIAL unique index. logSubscriptionEvent now
-- upserts with onConflict=stripe_event_id, ignoreDuplicates — so a re-driven
-- write silently no-ops instead of double-counting.
--
...
-- One analytics row per Stripe event. PARTIAL so the many legacy / cron rows
-- with a null stripe_event_id are exempt (a plain unique index would collapse
-- all nulls in some engines and reject legitimate null-keyed rows; the WHERE
-- clause makes the intent explicit and null-safe).
